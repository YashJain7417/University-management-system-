...
public class FacultyLeaveDetails extends JFrame implements ActionListener{ 
    ...
    FacultyLeaveDetails(){
        ...
        cemploy = new Choice();
        cemploy.setBounds(180,20,150,20);
        add(cemploy);
        ...
        try{
            Cons c = new Cons();
            ResultSet rs = c.s.executeQuery("select * from facultyleaves");
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == search){
            String query = "select * from facultyleaves where employid = '"+cemploy.getSelectedItem()+"'";
            try{
                Cons c = new Cons();
